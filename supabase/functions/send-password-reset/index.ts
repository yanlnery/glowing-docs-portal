...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!isValidEmail(email) || typeof resetUrl !== "string" || !resetUrl.startsWith("https://")) {
    ...
    const byEmail = rateLimit(`reset:email:${email.toLowerCase()}`, 3, 10 * 60_000);
    const byIp = rateLimit(`reset:ip:${ip}`, 3, 10 * 60_000);
    ...
    const recent = recentlySent("reset", email);
    if (recent.blocked) {
      return tooManyRequests(recent.retryAfter);
    }

    // Só envia se a conta existir; resposta genérica caso contrário
